...
func SetupRoutes(r *gin.Engine, cfg *config.Config, serverHandler *handlers.ServerHandler, consoleHandler *handlers.ConsoleHandler) {
	// Public routes
	r.GET("/health", handlers.PublicHealth)

	// WebSocket routes (no auth for now, can add token-based auth later)
	// Note: WebSocket connections don't support standard HTTP headers well,
	// so auth is typically done via query params or first message
	ws := r.Group("/ws")
	{
		ws.GET("/console/:server_id", consoleHandler.HandleConsoleWebSocket)
	}
